...
vec4 gradient(float x)
{
	...
	const vec4 c2 = vec4(0.0, 0.0, 0.0, 0); 	// black
	const vec4 c3 = vec4(0.8, 0.1, 0., 0.5); 	// red 
 float gs = pow(fbm(vec3(x,x,x)),2.2);
	const vec4 c4 = vec4(gs, gs, gs, 1.); 	// black
...
void main() {
    ...
    float rotx = time;//mouse.y*4.0;
    float roty = -time;//- mouse.x*4.0;

    float zoom = smoothstep(-1.,1., sin(time*0.2))*5.;

...
float smooth_noise(vec3 r) {
  vec3 integer = floor(r);
  vec3 fractional = r - integer;
...
float perlin(vec3 r) {
  return smooth_noise(r * .06125) * .5 +
         smooth_noise(r * .125) * .25 +
         smooth_noise(r * .25) * .125;
}

float sph(vec3 r, float a) {
  return length(r) - a;
}
...
float box(vec3 r, vec3 b) {
  vec3 d = abs(r) - b;
  return min(max(d.x,max(d.y,d.z)),0.0) +
         length(max(d,0.0));
}

float torus(vec3 r, vec2 t){
  vec2 q = vec2(length(r.xz)-t.x,r.y);
  return length(q)-t.y;
}

float cyl(vec3 r, float p, float c){return max(length(r.xz)-p, abs(r.y)-c);}

vec3 rep(vec3 r, vec3 q) { return mod(r,q) - 0.5*q; }
...
float head(vec3 r){
  rY(r,-0.45); rZ(r,0.25);
  r += vec3(2,-0.25,2.5);
  float top = box(r, vec3(0.3+impulse(0.4,r.z) * 0.15,clamp(0.2+ impulse(0.5,r.z) * 0.05,0.05,0.6),1.));
  float jaw = box(r-vec3(0,-0.4,0), vec3(0.3 + impulse(0.4,r.z) * 0.15,0.08+r.z*0.1,1.));
  r.x = abs(r.x);
  float ok  = sph(r-vec3(0.4,0.,0.5),0.1);
  return min(min(top,jaw),ok);
}
...
vec2 scene1(vec3 r) { 
  vec2 terrain = vec2(r.y + perlin(r*20.+tim*5.) * 2. + perlin(r*2.+tim*4.5)*5., 1.); 
...
void main() {
  ...
  gl_FragColor.xyz=abs(p.y) > 0.8? n4 : gamma(rm(p)*2.5)*(1.-length(p*0.2))-smoothstep(51.,54.,tim);
}

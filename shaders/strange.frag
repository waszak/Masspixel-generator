...
{
    float c = cos(0.5 * p.y + time);
    float s = sin(1.5 * p.y - time);
    mat2  m = mat2(c,-s,s,c);
...
float f(vec3 p)
{
	...
	float b = sphere(duplicate(opCheapBend( p ), vec3(1)), 0.45);
	return max(a,-b);
}
...
vec3 probe(vec3 p)
{
	...
	return vec3(perlin(p * 100. + time * 40.) * 0.07);
...
vec3 rm(vec3 ray)
{
	...
	return color * 2.2 * vec3(0.43, 0.123, 0.5);
}

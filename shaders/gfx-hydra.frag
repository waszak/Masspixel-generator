...
float world(vec3 p)
{
  ...
  for (int i=0;i<4;++i)
  {
	...
	return min(min(plan,c),min(t,drago));
}
...
void main()
{
   ...
   vec3 n = normal(p);
			//if (p.y < -2.2) 
				//{ color = vec3(0.8,0.7,0.7) * ao(p,n); break; }// * dot(n,ldir) * lp / ldst - 0.05 * exp(dst * 0.05); break; }
